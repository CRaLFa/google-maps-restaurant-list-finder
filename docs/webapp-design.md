...
- 都道府県と市区町村の並びは**全国地方公共団体コード順**。住所の並びとして見慣れた順序に揃える。
  表は総務省の Excel から起こした `cmd/server/muni-order.txt` (`コード<TAB>フルパス` を 1,965 行)。
...
- 件数は配下 (子孫) のエリア数を表示する。自身がエリアでも自分は数えない。0 なら出さない。
...
- **応答を返す前に送り切る。**
  当初は goroutine に逃がしてレスポンスを待たせない形にしたが、これは誤り。
  Cloud Run はリクエストの処理を終えると CPU をほぼ止めるので、逃がした goroutine は送られないまま終わりうる。
  しかも失敗ログすら残らず、取りこぼしに気付けない。
...
URL に JSON を POST するだけで済み、外部サービスの新規登録も要らない。
履歴が残るので「後から蓄積を確認する」目的も満たせる。
