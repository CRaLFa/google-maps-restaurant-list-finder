...
PR トリガが見るのは PR 全体の差分なので、コードを含む PR にドキュメントだけの commit を足しても走る。
...
[`data/seed.tsv`](../data/seed.tsv) の 2 列目に直接書く。
今後も同じ規則で増えるエリア種別なら、あわせて
[`scripts/locations.py`](../scripts/locations.py) の対応表
(`CITIES` / `WARDS` / `DISTRICTS` / `METRO`) にも足しておく
(`python3 scripts/locations.py` で自己チェックが走る)。
**シードの 2 列目は省略しないこと。**
省略すると `locations.py` の対応表からしか引けず、表に無いエリアは
「所在地未定義」で除外されて端末を触る前に落ちる。
...
検索語 (3 列目) は省略すると「所在地 + エリア名」で検索する。
**素の地名より、都道府県から始まる完全な住所の方が当たりやすい。**
素の地名は geo: がエリアページではなく駅・ランドマークのスポットページを開いてしまい、
カルーセルが無い = リスト無しと誤判定する原因になる
(「青山 東京」は空振りしたが、町名で引いた「東京都港区南青山」は 3 種類とも当たった)。
2 列目を住所表記で埋めておけば自動連結がそのまま住所になるので、
3 列目はランドマーク名・通称など自動連結が実在の住所にならないときだけ書く。

エリア名 (1 列目) は Google のリスト名と完全に一致させること。
カードの `content-desc` が `<エリア名>: トップリスト...` の形なので、
ここがずれると `カードが見つからない` で失敗する。
住所で検索したいだけなら 1 列目ではなく 3 列目を住所にする
(エリア名を `渋谷区渋谷` にするとカード名と合わず失敗する。
エリア名は `渋谷` のままで検索語を `東京都渋谷区渋谷` にする)。
...
このとき、既に 1 件以上あるエリアの欠けている種別 (`東京: トレンド` 等) は一時ファイルと無関係に毎回対象へ入る。
...
`--force` で既に座標があるエリアも取り直せる (補正式を変えたときの入れ替え用)。
`--dry-run` は Firestore に書かず値だけ表示、`--only <エリア名>` は指定エリアだけ処理する (複数指定可)。
なお全エリアを一気に流すと 7 件ごとに取りこぼしが出る (エリア固有の問題ではなく走行位置で決まる)。
失敗時はその場で 1 度取り直すが、それでも残ったエリアは `--force --only` で拾い直すこと。
...
件数は [README](../README.md) の冒頭を参照 (ここにも書くと更新が二重になる)。
1 エリアにつきトップリスト / トレンド / 地元で人気 の 3 種類を収録する。

- **端末にフォローして残しているのはトップリストのみ。** トレンド / 地元で人気 は共有 URL だけを保持している。
- 3 種類そろっていないエリアがあるのは、Google 側にそのリストが存在しないため (`東京: トレンド` 等。確認済み)。
  ただし後から Google 側に追加されることもある (`代官山町: トレンド` は 2026-09-13 に追加を確認)。
