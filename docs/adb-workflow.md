...
[`fetch_share_urls.py`](../fetch_share_urls.py) は**端末にフォロー済みのリストしか辿れない**ため、この欠落分は取得できない。
未フォローのリストはエリアページから開く必要がある。
これを自動化したのが [`fetch_missing_lists.py`](../fetch_missing_lists.py)。
...
```bash
cd <リポジトリルート>
DEV=100.64.1.35:37011 python3 fetch_missing_lists.py        # 全件
DEV=... OUT=test-missing.tsv MAX=2 python3 fetch_missing_lists.py  # 動作確認
```
