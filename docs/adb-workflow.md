...
- 実機で先頭数件の取得を確認済み (2026-07-25)。1 件あたり 20〜25 秒、461 件で 2.5〜3 時間。
...
- Firestore の `lists` を既知リストとして読み、**トップリストは残し**、
  それ以外で記録済みのものを削除する。
- **記録に無いリストは削除せず、共有 URL を収集して Firestore に記録**する
  (未記録のものを消さないための安全網)。
- 端末から削除した分は Firestore のドキュメントを消さず `followed` を false にする。
  共有 URL からの再フォローで復元できる。
...
記録済みのリストを「エリア名 → 種別」で集計すると、3 種類 (トップリスト / トレンド / 地元で人気) が揃っていないエリアがある。
2026-07-26 時点 (まだ `share-urls.tsv` が正データだった頃) で 168 エリア中 34 エリア・計 40 件が欠落していた。
