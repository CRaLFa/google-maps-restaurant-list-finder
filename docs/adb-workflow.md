...
- **【訂正】385 件は全件ではなかった。実際は 461 件。**
  後日 adb-clip 方式の [`fetch_share_urls.py`](../scripts/adb/fetch_share_urls.py) で全件収集したところ 461 件取得でき、
...
[`fetch_share_urls.py`](../scripts/adb/fetch_share_urls.py) が上記手順を全件自動化する。
adb-clip 方式で実装済み。リポジトリルートから実行する。

```bash
cd <リポジトリルート>
# 事前に adb-clip を push しておくこと (tools/adb-clip/README.md 参照)
DEV=192.0.2.1:42931 python3 scripts/adb/fetch_share_urls.py       # 全件
DEV=... MAX=5 OUT=data/test.tsv python3 scripts/adb/fetch_share_urls.py   # 動作確認 (5 件だけ別ファイルへ)
```
...
実装は [`delete_lists.py`](../scripts/adb/delete_lists.py)。以下は実機 (Xperia XQ-GE44 / Android 16) で確認した挙動。
...
[`fetch_share_urls.py`](../scripts/adb/fetch_share_urls.py) は**端末にフォロー済みのリストしか辿れない**ため、この欠落分は取得できない。
未フォローのリストはエリアページから開く必要がある。
これを自動化したのが [`fetch_missing_lists.py`](../scripts/adb/fetch_missing_lists.py)。

### 経路

1. エリア名で検索してエリアページを開く。
2. ボトムシートを下にスクロールすると、リストのカルーセルが現れる。
3. 目的のリストのカードをタップしてリスト詳細画面を開く。
4. トップリストのみ「リストを保存」をタップしてフォローする。
5. 画面右上の「共有」→ 共有シートの「クリップボードにコピー」で URL を取得する。

### 実証した挙動

- **リスト詳細画面の「共有」は未フォローでも押せる。**
  トレンド / 地元で人気 はフォローせずに共有 URL だけ取得できる
  (端末にはトップリストだけを残す運用に合わせている)。
- **エリアページを開くには `geo:` インテントを使う。**

  ```bash
  adb.exe -s $DEV shell am start -a android.intent.action.VIEW -d 'geo:0,0?q=%E4%B8%8B%E5%8C%97%E6%B2%A2'
  ```

  `am start -n ...MapsActivity` は**直前の画面を復元してしまう**。
  リスト詳細やスポット画面から始まると検索ボックスが存在せず、
  座標決め打ちで検索ボックスをタップすると地図上のスポットを開く事故になる。
  geo: インテントならどの画面からでも一発でエリアページに飛べ、
  クリップボード貼り付けも戻るキーの連打も不要になる。
- **カルーセルは横スクロール。** 3 枚が同時に見えるとは限らないので、
  目的のカードが見えなければカードの行を横に送って探す。
- カードは `content-desc` が `"下北沢: トップリスト. 🏆. Google Maps. 20 か所"` の形式で取れる。
- **エリア名が曖昧だと geo: は候補一覧を返す。**
  「恵比寿」では恵比寿通り (江東区) 等が混ざった一覧になり、カルーセルが存在しない。
  この場合はエリア名が完全一致する候補をタップしてから探し直す (`pick_search_result`)。
- **同名の別エリアが開かれることがある。**
  「伏見」は京都の伏見区が開かれてしまうため、`AREA_QUERY` で検索語を上書きする
  (`伏見` → `伏見 名古屋`)。
- **カードが見つからない場合、上記 2 つを潰してからでないと「存在しない」と判断できない。**
  初回実行で 4 件失敗したが、実際に存在しなかったのは 1 件だけで、
  残り 3 件は候補一覧・同名エリアが原因だった。
  Google の自動生成リストは全エリアで 3 種類揃うわけではないのも事実なので、
  カルーセルを横に送り切って全カードを列挙した上で判断すること。

### 実施結果 (2026-07-26)

40 件中 38 件を取得し、`share-urls.tsv` は 464 → 502 件になった。
トップリストと地元で人気は 168 エリア全部で揃った。

未取得は以下の 2 件。

- `代官山町: トレンド` — カルーセルにトップリストと地元で人気の 2 枚しかなく、**リストが存在しない**ことを確認済み。
- `東京: トレンド` — 「東京」が曖昧でエリアページに到達できず。調査を打ち切ったため存在の有無は未確定。

### 実行

```bash
cd <リポジトリルート>
DEV=192.0.2.1:37011 python3 scripts/adb/fetch_missing_lists.py        # 全件
DEV=... OUT=data/test-missing.tsv MAX=2 python3 scripts/adb/fetch_missing_lists.py  # 動作確認
```
