...
収集の話に入る前に、何をどう集めているかを先に書いておきます。

Google マップから取得しているのは、**リスト名・リストへの共有 URL・そのエリアの代表座標**の 3 つです。
Firestore にはこのほかに、リスト名を「エリア名」と「種類」に割った値、所在地と都道府県、更新日時も入れていますが、いずれも表示と並び替えのために手元で付けたメタデータです。
...
1 段階目は、**自分がフォローした約 460 件のリスト**です。
先に書いたとおり、Takeout でも「保存済み」ページでも全件を確認できない自分のデータを、手元に取り出したかったのが出発点でした。

2 段階目は、そこから漏れたエリアの掘り起こしです。
端末から辿れるのはフォロー済みのリストだけなので、未フォローのリストはエリア名で検索して開きます。
対象は東京 23 区、都道府県庁所在地、人口 20 万人以上の市、政令指定都市の各区で、325 件のシードを順に当てました。
空振りしたエリアも「リスト無しと確認済み」として記録しています。
